...
import type { User } from "../services/startupService"
...
export const useStartups = () => {
  ...
  const updateStartup = async (id: number, data: Partial<Startup>) => {
    try {
      const updatedStartup = await startupService.updateStartup(id, data)
      setStartups((prev) =>
        prev.map((startup) => (startup.id === id ? updatedStartup : startup))
      )
  ...
  const addTeamMember = async (startupId: number, data: {
    user: number
    role_in_team: string
  }) => {
    try {
      ...
      setStartups((prev) =>
        prev.map((s) => (s.id === startupId ? startup : s))
      )
    } catch (err) {
...
export const useStartup = (id: number) => {
  ...
  const addTeamMember = async (data: Partial<TeamMember> & { member_name?: string }) => {
    try {
